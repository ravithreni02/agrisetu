...
import { CalendarCheck, Clock, CheckCircle, UsersRound } from "lucide-react";
...
const GroupLeaderDashboard = () => {
  ...
  const available = Object.entries(availability).filter(([, v]) => v).map(([k]) => new Date(k + "T00:00:00"));
  const unavailableDates = Object.entries(availability).filter(([, v]) => !v).map(([k]) => new Date(k + "T00:00:00"));
    ...
    <div className="p-6 pb-24 md:pb-6 space-y-6">
      <div>
        <h1 className="text-2xl font-bold flex items-center gap-2">
          <UsersRound className="h-6 w-6 text-primary" /> Group Leader Dashboard
        </h1>
        <p className="text-muted-foreground text-sm">Manage your group's schedule and bookings</p>
      </div>
      ...
      <div className="grid md:grid-cols-2 gap-6">
        ...
        <Card className="rounded-2xl border-border/50">
          <CardHeader className="pb-3">
            <CardTitle className="text-lg flex items-center gap-2">
              <CalendarCheck className="h-5 w-5 text-primary" /> Group Availability
            </CardTitle>
            <p className="text-xs text-muted-foreground">Set group schedule: click to toggle</p>
          </CardHeader>
          <CardContent>
            ...
            <CalendarUI
              ...
              modifiers={{ available, unavailable: unavailableDates }}
              modifiersClassNames={{
        ...
        <Card className="rounded-2xl border-border/50">
          <CardHeader className="pb-3">
            <CardTitle className="text-lg flex items-center gap-2">
              <CalendarCheck className="h-5 w-5 text-primary" /> Group Booking Requests
            </CardTitle>
          </CardHeader>
          <CardContent className="space-y-3 max-h-96 overflow-y-auto">
            {bookings.length === 0 ? (
              <p className="text-sm text-muted-foreground text-center py-8">No group bookings yet</p>
            ) : bookings.map((b) => (
